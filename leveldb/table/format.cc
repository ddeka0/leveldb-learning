...
Status Footer::DecodeFrom(Slice* input) {
  ...
  auto original_input = input->ToString();
  auto size_before = input->size();
  Status result = metaindex_handle_.DecodeFrom(input);
  auto size_after = input->size();
  if (result.ok()) {
    MYPRINT << "Decoded metaindex handle [offset: "
            << metaindex_handle_.offset()
            << " size: " << metaindex_handle_.size() << "] from "
            << ByteBufferToString(
                   original_input.substr(0, size_before - size_after))
            << std::endl;
    original_input = input->ToString();
    size_before = input->size();
    result = index_handle_.DecodeFrom(input);
    size_after = input->size();
  }
  if (result.ok()) {
    MYPRINT << "Decoded index handle [offset: " << index_handle_.offset()
            << " size: " << index_handle_.size() << "] from "
            << ByteBufferToString(
                   original_input.substr(0, size_before - size_after))
            << std::endl;
