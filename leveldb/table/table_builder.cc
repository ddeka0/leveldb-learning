...
const std::string kMetaIndexBlockStr = "MetaIndexBlock";
...
struct TableBuilder::Rep {
  Rep(const Options& opt, WritableFile* f)
      : options(opt),
        ...
        file(f),
        offset(0),
        data_block(&options, kDataBlockStr),
        index_block(&index_block_options, kIndexBlockStr),
        num_entries(0),
        closed(false),
...
void TableBuilder::Flush() {
  ...
  MYPRINT << "Calling WriteBlock" << std::endl;
...
void TableBuilder::WriteBlock(BlockBuilder* block, BlockHandle* handle) {
  ...
  auto raw_str = raw.ToString();
  MYPRINT << block->BlockTypeName()
          << " contents: " << ByteBufferToString(raw_str) << std::endl;
...
void TableBuilder::WriteRawBlock(const Slice& block_contents,
  ...
  if (r->status.ok()) {
    ...
    MYPRINT << "Adding block trailer: "
            << ByteBufferToString(Slice(trailer, kBlockTrailerSize).ToString())
            << " to SSTable file: " << r->file->FileName() << std::endl;
    r->status = r->file->Append(Slice(trailer, kBlockTrailerSize));
    if (r->status.ok()) {
      r->offset += block_contents.size() + kBlockTrailerSize;
      MYPRINT << "Current offset is: " << r->offset
              << " in the SSTable file: " << r->file->FileName() << std::endl;
    }
  }
}
...
Status TableBuilder::Finish() {
  ...
  if (ok() && r->filter_block != nullptr) {
    MYPRINT << "Writing filter block to " << r->file->FileName() << std::endl;
  ...
  if (ok()) {
    BlockBuilder meta_index_block(&r->options, kMetaIndexBlockStr);
    if (r->filter_block != nullptr) {
    ...
    MYPRINT << "Writing meta index block to " << r->file->FileName()
            << std::endl;
  ...
  if (ok()) {
    ...
    MYPRINT << "Writing index block to " << r->file->FileName() << std::endl;
    WriteBlock(&r->index_block, &index_block_handle);
  }

  // Write footer
  if (ok()) {
    MYPRINT << "Appending footer to " << r->file->FileName() << std::endl;
    Footer footer;
